...
//                }
                DoubleTensor[] directions = Directions.TGLC1.getDirections();
                double[] direction_weights = Directions.TGLC1.getDirectionWeights();
                ...
                AngularScalarFunction scatter = new AngularScalarFunction()
                {
                        ...
                        public double value(DoubleTensor posInSpace, DoubleTensor posOnSphere1, DoubleTensor posOnSphere2)
                        {
                                ...
                                double osc = Math.cos(posInSpace.x()*4)*Math.cos(posInSpace.y()*4);
                                return 10+9.99999*Math.pow(Math.abs(osc),1)*Math.signum(osc);
                        }
                };
                ...
                AngularScalarFunction absorb = new AngularScalarFunction()
                {
                        ...
                        {
                                double abs = 0.2;
                                if(Math.cos(posInSpace.x()*4)*Math.cos(posInSpace.y()*4)<0)
                                        abs = 10;
                                return scatter.value(posInSpace,posOnSphere1,posOnSphere2)*1+abs;
                        }
                };
